% opt_ctr keeps track of what step in the optimization the system is
% infbest
...
if opt_ctr == 0
    ...
    tempdir = 'C:\Users\HPLC\Dropbox (MIT)\MoD Files\MATLAB';
    %tempdir = 'C:\Users\Admin\Dropbox (MIT)\MoD Files\MATLAB';
    file = strcat(matlabsavefile,'snobfitonly');% filename for storing intermediate data;
    %'C:\Users\HPLC\Dropbox (MIT)\MoD Files\MATLAB\reductive_amination_snobfit'; 
    %file = 'C:\Users\Admin\Dropbox (MIT)\MoD Files\MATLAB\reductive_amination_snobfit'; % filename for storing intermediate data;
    % after each call to SNOBFIT) are stored
    % in <file>.mat
    fcn = 'reactor_obj_fun';         % objective function
    indices = find(Opt_params);
    %Box_pos = [T1_span; T2_span; T3_span; T4_span; T5_span;...
    %    p1_span; p2_span;p3_span; p4_span; p5_span; p6_span;];
    Box_pos = [T_span;p_span];
    Box = Box_pos(indices,:);
...
%             end
            ...
            if isFeasible
                ...
                if not(incr_hplc_peak)
                    f(req_ctr,:) = [-inf sqrt(eps)];
                else
                    f(req_ctr,:) = [inf sqrt(eps)];
                end

...
sig_num = 1;
ret_time_react_prod2 = 0;
area_react_prod2 = 0;
% Find all peaks between minimum and maximum residence time
if not(isempty(peak_data))
peak_range = find(peak_data(:,8) - hplc_peak_start2  >= 0 & hplc_peak_end2 - peak_data(:,8) >= 0);
...
for peak_num = peak_range'
    ...
    if peak_data(peak_num,1) == sig_num
        %area_react_prod = peak_data(peak_num,11)+area_react_prod;
        % If greater than previous area, accept
         if (peak_data(peak_num,11) > area_react_prod2) 
             ret_time_react_prod2 = peak_data(peak_num,8);
             area_react_prod2 = peak_data(peak_num,11);
         end
    end
end
end

ee = (area_react_prod -area_react_prod2)/(area_react_prod + area_react_prod2);

Conc = 0;
if area_ISTD_254 == 0
   Conc = area_react_prod*area_react_prod/area_react_prod2; 
   %%Remove Next Line if not a simulation
   %Conc = P1*P2/T4/P3*P4*T2;
else
   Conc = (area_react_prod)/area_ISTD_254*area_react_prod/area_react_prod2; 
    %%Remove Next Line if not a simulation
   %Conc = P1*P2/P5;
end
...
HPLC_Area(end +1,:) = [CurrExp area_react_prod area_react_prod2 area_ISTD_254 area_react_prod/area_react_prod2];
